...
Image input supports one PNG or JPEG per request using an OpenAI `image_url`
data URL:

```python
import base64

with open("part-label.jpg", "rb") as image_file:
...
resp = client.chat.completions.create(
    ...
    messages=[{
        "role": "user",
        "content": [
            {"type": "text", "text": "Read the exact industrial part number."},
            {
                "type": "image_url",
...
Remote image URLs are not fetched. Send a base64 data URL. Images are limited
to 20 MB and the consumer Copilot transport accepts one image per request.
