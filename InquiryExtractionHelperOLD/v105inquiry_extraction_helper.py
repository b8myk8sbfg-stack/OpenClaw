...
VERSION = "v1.05-INQUIRY-EXTRACTION-HELPER-COMPACT-QTY-FIX"
...
def preprocess_body(text: str) -> str:
    ...
    body = re.sub(
        r"\s+(OMRON|SMC|BURKERT|BÜRKERT|PANASONIC|THK|LOCTITE|KEYENCE|FESTO|SICK|IFM|PARKER|ABB|SIEMENS)\s+",
        r"\n\1 ",
        body,
        flags=re.I
    )
...
def is_dangerous_candidate(candidate: str, part_no: str) -> bool:
    ...
    dangerous = {
        ...
        "TIMER", "RELAY", "SENSOR", "PRESSURE", "VALVE", "PLC", "UNIT", "10S", "5S", "2M"
    }
...
def extract_brand_from_text(text: str) -> str:
    ...
    brands = [
        ...
        "KEYENCE", "FESTO", "SICK", "IFM", "PARKER", "ABB", "SIEMENS"
    ]
...
def guess_part_no(search_text: str) -> str:
    ...
    # Prefer part-like token containing both letters and digits, with optional dash.
    part_like = re.findall(r"\b(?=[A-Z0-9\-]*[A-Z])(?=[A-Z0-9\-]*\d)[A-Z0-9]+(?:-[A-Z0-9]+)+\b", text)
    if part_like:
        return part_like[-1].upper()

    # Then compact alpha-number token like DP102 or MTNS000148AA.
    compact = re.findall(r"\b(?=[A-Z0-9]*[A-Z])(?=[A-Z0-9]*\d)[A-Z0-9]{4,}\b", text)
    if compact:
        return compact[-1].upper()

    # Remove descriptive words and retry.
    text = re.sub(
        r"\b(TIMER|RELAY|SENSOR|PRESSURE|VALVE|PLC|UNIT|PRICE|QUOTE|ITEM|URGENTLY|SUPPLIER)\b",
        " ",
        text
    )
...
def build_candidates(part_no: str, search_text: str) -> List[str]:
    ...
    # Also add normalized dash/space variants for items like DP-102.
    if "-" in part_no:
        candidates.append(part_no.replace("-", " "))
...
def extract_clean_items_from_text(text: str) -> List[Dict]:
    ...
    compact_dash_qty_pattern = re.compile(
        r"(?:^|\n)\s*(.*?)\s*-\s*(\d+)\s*(?:PCS|PC|PCE|UNIT|UNITS|NOS)\b",
        re.I
    )
    ...
    for raw_item, qty in compact_dash_qty_pattern.findall(body):
        ...
        if not re.search(
            r"\b(OMRON|SMC|BURKERT|BÜRKERT|PANASONIC|THK|LOCTITE|KEYENCE|FESTO|SICK|IFM|PARKER|ABB|SIEMENS)\b",
            raw_item,
            re.I
        ):
            continue
    ...
    line_dash_qty_pattern = re.compile(
        r"(?:^|\n)\s*(?:\d+\s*[\.\)]\s*)?(.*?)\s*-\s*QTY\s*[;:\-]?\s*(\d+)\s*(?:PCS|PC|PCE|UNIT|UNITS|NOS)?",
        re.I
    )

    for raw_item, qty in line_dash_qty_pattern.findall(body):
        raw_item = clean_line(raw_item)
    ...
    # Format:
    # Omron H3Y... Qty : 2 Unit
    brand_qty_pattern = re.compile(
        r"\b(OMRON|SMC|BURKERT|BÜRKERT|PANASONIC|THK|LOCTITE|KEYENCE|FESTO|SICK|IFM|PARKER|ABB|SIEMENS)\b\s+"
...
Pls help me quote below item urgently :-
