...
VERSION = "v1.07-NON-STANDARD-TECHNICAL-ROUTING"
...
def extract_structured_rfq_items(body_upper):
    ...
    for brand, part_no, qty in pattern_with_brand.findall(body_upper):
        ...
        rfq_items.append({
            ...
            "source": "LAYER2_WITH_BRAND"
        })

    id_pattern = re.compile(
        r'\b(OMRON|SMC|BURKERT|BURKERT|BÜRKERT|KEYENCE|FESTO|SICK|IFM|PARKER|ABB|SIEMENS)\s+'
    ...
    for brand, part_no, qty in id_pattern.findall(body_upper):
        brand = brand.strip().upper().replace("BÜRKERT", "BURKERT")
        part_no = part_no.strip().upper()
        qty = int(qty)
        norm = normalize_part(part_no)

        if norm not in {x["norm"] for x in rfq_items}:
            rfq_items.append({
                "brand": brand,
                "part_no": part_no,
                "qty": qty,
                "norm": norm,
                "source": "LAYER2_BRAND_ID_FORMAT"
            })
    ...
    existing_norms = {x["norm"] for x in rfq_items}
    ...
    for part_no, qty in pattern_without_brand.findall(body_upper):
        ...
        rfq_items.append({
            ...
            "qty": qty,
            "norm": norm,
            "source": "LAYER2_NO_BRAND_DEFAULT"
        })
    ...
    try:
        ...
        for msg in messages:
            ...
            for missing in missing_layer2_items:
                ...
                # IMPORTANT:
                # Layer 2 item means the customer provided a part/model,
                # but it was not found in warehouse stock matching.
                # These are NON-STANDARD items and should go to technical,
                # NOT supplier RFQ.
                ...
                print(f"   🧩 Non-standard item detected and routed to technical.")
                print(f"      Brand: {brand}")
